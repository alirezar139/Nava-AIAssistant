...
این سند نصب، اجرا، بررسی سلامت، build و رفع خطاهای رایج نوا را در
...
PWA رابط کاربری نوا را مثل یک اپ مستقل در Chrome یا Microsoft Edge نصب می‌کند.
...
4. بعد از نصب، نوا از Start Menu و Taskbar مثل یک پنجره مستقل باز می‌شود.
...
علاوه بر خروجی ترمینال، API هر درخواست (متد، مسیر، کد وضعیت، زمان پاسخ،
traceId) و هر خطای مدیریت‌نشده را در فایل روزانه زیر ذخیره می‌کند:

```text
server/logs/app-YYYY-MM-DD.log
```

این فایل‌ها در Git ردیابی نمی‌شوند. برای ریشه‌یابی خطاهایی مثل قطعی
دیتابیس یا خطای ۵۰۰، ابتدا همین فایل روز جاری را بررسی کنید؛ traceId هر
پاسخ هم در هدر `X-Trace-Id` برمی‌گردد تا با خط مربوطه در لاگ تطبیق داده شود.

همین لاگ از تب «گزارش لاگ» در پنل مدیریت هم قابل مشاهده و فیلتر (بر اساس
تاریخ، سطح و متن پیام) است؛ نیازی به دسترسی مستقیم به فایل سیستم نیست.
