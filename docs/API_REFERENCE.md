...
در توسعه، `proxy.conf.json` این مسیر را به `http://127.0.0.1:4300` منتقل می‌کند.
...
| ------------ | ------------- | ----- |
| `projectKey` | `default`     | کد پروژه‌ای که درختواره آن باید خوانده شود. |
| `mode`       | `active`      | مقدار `active` نسخه قابل استفاده کاربر را برمی‌گرداند. مقدار `draft` برای پیش‌نویس مدیریتی است. |

صفحه کاربر باید فقط نسخه فعال را بخواند. پنل مدیر برای ویرایش از `mode=draft` استفاده می‌کند و فقط در زمان نهایی‌سازی با `mode=active` ذخیره می‌کند.

مدل ذخیره‌سازی:

- `projectKey` مشخص می‌کند درختواره مربوط به کدام پروژه است.
- `mode=active` نسخه قابل استفاده کاربران و ثبت تیکت است.
- `mode=draft` نسخه ویرایشی مدیر است.
- در ArangoDB، نودها و ارتباط‌ها با `projectKey` و `treeMode` ذخیره می‌شوند تا نسخه‌های active و draft یک پروژه روی هم نوشته نشوند.
- metadata نسخه‌ها در `troubleshooting_tree_versions` نگهداری می‌شود و شامل شماره نسخه، تعداد نود، تعداد ارتباط و زمان به‌روزرسانی است.
- پاسخ `mode=active` برای کاهش فشار روی دیتابیس cache کوتاه‌مدت دارد. پاسخ `mode=draft` برای جلوگیری از نمایش پیش‌نویس stale در زمان ویرایش cache مرورگر نمی‌شود.
