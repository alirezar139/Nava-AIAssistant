...
پشتیبانی از طریق Jira API در نظر گرفته می‌شود.
...
در صورت حل نشدن مشکل، سامانه باید قابلیت ساخت تیکت Jira را داشته باشد. نوع تیکت،
فیلدهای اجباری و روش احراز هویت (`Token`، `Basic Auth` یا `OAuth`) وابسته به
مستندات و دسترسی کارفرما است.
...
- پس از دریافت تنظیمات Jira، یک تیکت آزمایشی با موفقیت ساخته شود.
